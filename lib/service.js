...
export class WebPushService extends Service {
    ...
    async sendTelegram(payload) {
        ...
        if (!this.config.enabled || !tg.enabled || !tg.botToken || !tg.chatId)
            return;
        try {
            const response = await axios.post(`https://api.telegram.org/bot${tg.botToken}/sendMessage`, { chat_id: tg.chatId, text: payload.title }, { timeout: 8000, proxy: this.axiosProxy() });
            if (!response.data?.ok)
                this.ctx.logger.warn('[web-push] telegram rejected:', response.data?.description ?? 'unknown');
        }
        catch (error) {
            const err = error;
    ...
    axiosProxy() {
        ...
        if (envProxy)
            return undefined; // let axios use the env proxy verbatim
        const explicit = this.resolveProxy();
        if (!explicit)
            return false; // truly direct
        try {

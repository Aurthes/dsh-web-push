...
function respond(res: ServerResponse, status: number, contentType: string, body: string, extraHeaders: Record<string, string> = {}): void {
...
export default function webPushPlugin(ctx: Context, config?: Parameters<WebPushService['getConfig'] extends never ? never : never> | any) {
  ...
  const log = { warn: (...args: unknown[]) => ctx.logger.warn(...(args as [string, ...unknown[]])) }

  // HTTP routes for the browser: service worker, manifest, icon.
  const webServer = serviceOf(ctx, 'webServer') as {
  ...
  let disposeRpc: () => void = () => {}
  const httpRoute = installPushHttpRoute({ connection, webServer } as any, bridge, log)
  if (httpRoute) {
    disposers.push(httpRoute)
  } else {
    disposeRpc = installPushRpc(connection?.rpc as any, bridge, log)
  }
...
;(webPushPlugin as unknown as { inject: string[] }).inject = inject

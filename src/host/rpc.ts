 ...
 */
export function installPushRpc(
...
function pushRpcHandler(bridge: PushRpcBridge) {
  return async (endpoint: string, payload: any = {}, signal?: { aborted?: boolean }): Promise<unknown> => {
    ...
    switch (endpoint) {
      ...
      case PUSH_ENDPOINTS.configSet: {
        if (payload === null || typeof payload !== 'object') return fail('push.config.set expects an object payload')
        return ok(bridge.write(payload as Partial<PluginConfig>))
      }
      ...
      case PUSH_ENDPOINTS.unsubscribe: {
        ...
        if (typeof endpoint !== 'string' || !endpoint) return fail('push.unsubscribe expects { endpoint }')
...
function isTrustedLoopbackRequest(req: IncomingMessage): boolean {
  ...
  if (!LOOPBACK_HOSTNAMES.has(host.split(':')[0])) return false
 ...
 */
export function installPushHttpRoute(
  ctx: { connection?: { requestRejection?: (req: IncomingMessage) => number | undefined }; webServer?: { register(route: unknown): unknown } },
  bridge: PushRpcBridge,
  log: { warn?: (...args: unknown[]) => void } = {},
): (() => void) | null {
  const webServer = ctx?.webServer
  if (!webServer || typeof (webServer as { register?: unknown }).register !== 'function') return null
  ...
  const route = {
    ...
    handler: async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
      ...
      for await (const chunk of req) {
        ...
        if (received > PUSH_RPC_BODY_MAX) { tooLarge = true; break }
  ...
  const registered = (webServer as { register(route: unknown): unknown }).register(route)
  const cleanup = typeof registered === 'function'
    ? () => { try { (registered as () => void)() } catch { /* already disposed */ } }
    : registered && typeof (registered as Promise<unknown>).then === 'function'
      ? (() => { let done = false; return () => { if (done) return; done = true; void (registered as Promise<() => void>).then((d) => { try { d?.() } catch { /* already disposed */ } }).catch(() => {}) } })()
      : () => {}

...
type EventLike = { type?: string; data?: any; time?: number }
...
export class WebPushService extends Service {
  ...
  constructor(ctx: Context, config?: Partial<PluginConfig>) {
    ...
    if (this.config.enabled) this.registerEventListeners()

    // `send` on ctx is owned by dsh-notify-plugin; expose ours prefixed.
    ctx.mixin('webPush', { send: 'webPushSend' } as any)
  }

  /** Effective runtime config (persisted edits already merged by the entry). */
  getConfig(): PluginConfig {
    return { ...this.config, events: { ...this.config.events }, telegram: { ...this.config.telegram } }
  }
  ...
  updateConfig(partial: Partial<PluginConfig>): PluginConfig {
    const next: PluginConfig = {
      ...
      telegram: {
        enabled: typeof partial.telegram?.enabled === 'boolean' ? partial.telegram.enabled : this.config.telegram.enabled,
        botToken: typeof partial.telegram?.botToken === 'string' ? partial.telegram.botToken : this.config.telegram.botToken,
        chatId: typeof partial.telegram?.chatId === 'string' ? partial.telegram.chatId : this.config.telegram.chatId,
      },
    }
    this.config = next
    saveConfig(next)
    if (next.enabled) this.registerEventListeners()
  ...
  addSubscription(subscription: Record<string, unknown>, label: string, origin?: string): void {
    ...
    if (!keys || typeof keys !== 'object') throw new Error('subscription.keys (p256dh/auth) is required')
    this.subscriptions = [
      ...this.subscriptions.filter((s) =>
        s.endpoint !== endpoint && !(s.label === label && s.origin !== origin)),
      {
        endpoint,
  ...
  removeSubscription(endpoint: string): boolean {
    ...
    if (this.subscriptions.length !== before) saveSubscriptions(this.subscriptions)
   ...
   */
  async send(payload: NotifyPayload): Promise<{ sent: number; removed: number; errors: string[] }> {
    let sent = 0
    let removed = 0
    const errors: string[] = []
    if (!this.config.enabled) return { sent, removed, errors }
    if (this.subscriptions.length === 0) return { sent, removed, errors }
    ...
    if (proxy) sendOptions.proxy = proxy
    await Promise.all(this.subscriptions.map(async (stored) => {
      try {
        await webpush.sendNotification(
          stored.subscription as unknown as Parameters<typeof webpush.sendNotification>[0],
          body,
          sendOptions as Parameters<typeof webpush.sendNotification>[2],
        )
    ...
    // Mirror the same one-line payload to Telegram when configured: it rides
    // the Bot API (token + chat id), so unlike the native channel it survives
    // every restart and domain rotation without re-authorization.
  ...
  private async sendTelegram(payload: NotifyPayload): Promise<void> {
    ...
    if (!this.config.enabled || !tg.enabled || !tg.botToken || !tg.chatId) return
    try {
      const response = await axios.post(
        `https://api.telegram.org/bot${tg.botToken}/sendMessage`,
        { chat_id: tg.chatId, text: payload.title },
        { timeout: 8000, proxy: this.axiosProxy() },
      )
      if (!response.data?.ok) this.ctx.logger.warn('[web-push] telegram rejected:', response.data?.description ?? 'unknown')
    } catch (error) {
      const err = error as { code?: string; message?: string }
      this.ctx.logger.warn('[web-push] telegram send failed:', err.code ?? '', err.message ?? err)
    }
  }
   ...
   */
  private axiosProxy(): { host: string; port: number; protocol: string } | false | undefined {
    const envProxy = (process.env.https_proxy ?? process.env.HTTPS_PROXY ?? '').trim()
    if (envProxy) return undefined // let axios use the env proxy verbatim
    const explicit = this.resolveProxy()
    if (!explicit) return false // truly direct
    try {
  ...
  async sendTest(): Promise<{ sent: number; removed: number; errors: string[] }> {
  ...
  private mergeConfig(partial: Partial<PluginConfig>): PluginConfig {
    ...
    const merged: PluginConfig = {
      ...
      telegram: { ...DEFAULT_CONFIG.telegram, ...(partial.telegram ?? {}), ...(persisted?.telegram ?? {}) },
    }
    return merged
  }

  private generateVapid(): { publicKey: string; privateKey: string } {
  ...
  private registerEventListeners(): void {
    ...
    this.ctx.on('session/event' as any, async (session: SessionLike, event: EventLike) => {
      try {
  ...
  private async handleSessionEvent(session: SessionLike, event: EventLike): Promise<void> {
   ...
   * session.
   */
  private async handleTurnEnd(session: SessionLike, event: EventLike): Promise<void> {
    if (!this.config.events.conversationCompleted && !this.config.events.conversationFailed && !this.config.events.conversationPaused) return
    ...
    if (reason === 'completed' || reason === 'max-tokens') {
      if (!this.config.events.conversationCompleted) return
      await this.send({ title: `✅ ${name}`, tag, ...link })
    } else if (reason === 'error') {
      if (!this.config.events.conversationFailed) return
      await this.send({ title: `❌ ${name}`, tag, ...link })
    } else {
      if (!this.config.events.conversationPaused) return
      await this.send({ title: `⏸️ ${name}`, tag, ...link })
    }
  }

  /** Click-target fields shared by every session-scoped push. */
  private deepLink(session: SessionLike): { url: string; session?: string } {
    const id = typeof session?.id === 'string' ? session.id : ''
    return id ? { url: `/?session=${encodeURIComponent(id)}`, session: id } : { url: '/' }
  }

  private async handleUserQuestion(session: SessionLike, event: EventLike): Promise<void> {
    if (!this.config.events.confirmationRequired) return
  ...
  private async handleApprovalRequest(session: SessionLike, event: EventLike): Promise<void> {
    if (!this.config.events.authorizationRequired) return
  ...
  private async handleTodos(session: SessionLike, todos: unknown): Promise<void> {
    if (!this.config.events.todoProgress) return
    if (!Array.isArray(todos) || todos.length === 0) return
    const items = todos as Array<{ content?: string; status?: string }>
    const done = items.filter((t) => t?.status === 'completed').length
    const signature = JSON.stringify(items.map((t) => `${t?.status ?? ''}:${t?.content ?? ''}`))
    if (this.lastTodoSignature.get(session?.id) === signature) return
    this.lastTodoSignature.set(session?.id, signature)

    const icons: Record<string, string> = { completed: '✅', in_progress: '🔄' }
    const lines = [`📊 进度: ${done}/${items.length} 已完成`]
    for (const item of items.slice(0, 10)) {
      lines.push(`${icons[item?.status ?? ''] ?? '⬜'} ${item?.content ?? ''}`)
    }
    if (items.length > 10) lines.push(`… 共 ${items.length} 项`)
    const workspace = this.workspaceOf(session)
    await this.send({
  ...
  private workspaceOf(session: SessionLike): string | undefined {
    const cwd: string | undefined = session?.header?.cwd
    return cwd ? cwd.split('/').filter(Boolean).pop() || cwd : undefined
  }

  private isSubagentSession(session: SessionLike): boolean {
